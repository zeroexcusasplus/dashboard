(function ($) {
  ...
  var defaults = {
    ...
    widget: {
      wrapper: {
        tag: 'div',
        class: 'input-group custom-date-wrapper'
      },
      wrapperYear: {
        use: true,
        tag: 'span',
        class: 'input-group-addon custom-date-year'
      },
      wrapperMonth: {
        use: true,
        tag: 'span',
          class: 'input-group-addon custom-date-month'
      },
      wrapperDay: {
        use: true,
        tag: 'span',
          class: 'input-group-addon custom-date-day'
      },
      selectYear: {
    ...
    var createHtmlWidget = function () {
      ...
      if (bsBD.settings.futureDates && bsBD.settings.maxYear != todayYear) {
        ...
        for (var i = startYear; i >= endYear; i--) {
            var yearSelected = document.getElementById("dateOfBirth_yearSelected").value;
            if (yearSelected == i) {
        ...
        for (var j = 0; j < 12; j++) {
            var monthSelected = document.getElementById("dateOfBirth_monthSelected").value;
            if (j+1 == monthSelected) {
                $("<option selected></option>")
                    .attr("value", j+1)
        ...
        for (var k = 1; k < 32; k++) {
            var daySelected = document.getElementById("dateOfBirth_daySelected").value;
            if (k == daySelected) {
                $("<option selected></option>")
                    .attr("value", k)
                    .text(k)
